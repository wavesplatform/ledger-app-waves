    ...
    if (wire_type == PB_WT_STRING &&
      ...
      while (substream.bytes_left) {
        if ((size_t)*size + 1 > allocated_size) {
          ...
          if (!allocate_field(&substream, field->pField, field->data_size,
                              allocated_size)) {
            status = false;
            break;
          }
...
static bool checkreturn decode_callback_field(pb_istream_t *stream,
  ...
  if (wire_type == PB_WT_STRING) {
    ...
    do {
      ...
      if (!((bool (*)(pb_istream_t * istream, pb_ostream_t * ostream,
                      const pb_field_iter_t *field))(
              PIC(field->descriptor->field_callback)))(&substream, NULL, field))
