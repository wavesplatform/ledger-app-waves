...
def sign_tx(tx: str, data_version: int, client: WavesSignData, navigator: Navigator, test_name: str,
                      default_screenshot_path: str, public_key: bytes, tx_type: int) -> None:
    ...
    response = run_with_delay(lambda: client.sign_tx(path=path, tx_type=tx_type, data_version=data_version, data=sign_data, chain_id=chain_id),  review_approve, delay=1.0)
